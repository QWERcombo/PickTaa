...
@implementation XSWJVARegexPattern

//  企业发票信息：抬头类型：企业发票（必填）、发票抬头（必填 2-80字符 汉字英文）、纳税人识别号（必填 企业 数字字母 15-20）、发票内容、开票金额、开户银行（非必填 2-80字符）、银行账户（非必填2-80数字 字符）、单位电话（非必填 固话/手机 数字）、单位地址（非必填1-200字符 ）；

// 企业发票,发票抬头（必填 2-80字符 汉字英文）
+ (BOOL)validateCompanyInvoiceTitle:(NSString *)companyInvoiceTitle {
    if (!companyInvoiceTitle || companyInvoiceTitle.length == 0)
        return NO;
    
    NSString *regex = @"^[\u4e00-\u9fa5a-zA-Z]{2,80}$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
    
    return [pred evaluateWithObject:companyInvoiceTitle];
}

//纳税人识别号（必填 企业 数字字母 15-20）
+ (BOOL)validateTIN:(NSString *)TIN {
    if (!TIN || TIN.length == 0)
        return NO;
    ...
    return [pred evaluateWithObject:TIN];
}

// 发票内容、开票金额、开户银行（非必填 2-80字符）
+ (BOOL)validateInvoiceCTY:(NSString *)invoiceCTY {
    if (!invoiceCTY || invoiceCTY.length == 0)
        return YES;
    
    if (invoiceCTY.length>=2 && invoiceCTY.length<=80) {
        return YES;
    }
    ...
    //    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
    //
    //    return [pred evaluateWithObject:invoiceCTY];
}
...
- (BOOL)validateBankCardNumber:(NSString *)bankCardNumber {
    ...
    NSString *regex2 = @"^(\\d{2,80})";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex2];
    
    return [pred evaluateWithObject:bankCardNumber];
}

// 单位电话（非必填 固话/手机 数字）
+ (BOOL)validateCompanyPhone:(NSString *)companyPhone {
    if (!companyPhone || companyPhone.length == 0) {
        return YES;
    }
    
    NSString *regex2 = @"((\\d{11})|^((\\d{7,8})|(\\d{4}|\\d{3})-(\\d{7,8})|(\\d{4}|\\d{3})-(\\d{7,8})-(\\d{4}|\\d{3}|\\d{2}|\\d{1})|(\\d{7,8})-(\\d{4}|\\d{3}|\\d{2}|\\d{1}))$)";
    //    NSString *regex2 = @"^(\\(\\d{3,4}-)|\\d{3.4}-)?\\d{7,8}$";
    //    NSString *regex2 = @"\\d{3}-\\d{8}|\\d{4}-\\d{7}";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex2];
    
    return [pred evaluateWithObject:companyPhone];
}

// 单位地址（非必填1-200字符 ）；
+ (BOOL)validateCompanyAddress:(NSString *)companyAddress {
    if (!companyAddress || companyAddress.length == 0)
        return YES;
    
    if (companyAddress.length>=2 && companyAddress.length<=80) {
        return YES;
    }
    ...
    //    }
    //
    //    NSString *regex = @"[\u4e00-\u9fa5a-zA-Z0-9]{2,80}";
    //    NSPredicate *bankCardPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
    //
    //    return [bankCardPredicate evaluateWithObject:companyAddress];
}
...
+ (BOOL)validateIsValidMailbox:(NSString *)targetString {
    ...
    NSString *regex = @"^[A-Za-z0-9\u4e00-\u9fa5\\._\\-]+@[a-zA-Z0-9_\\-\u4e00-\u9fa5]+(\\.[a-zA-Z0-9_\\-]+)+$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
...
//    NSString *phoneRegex = @"^((1[3,5,8][0-9])|(14[5,7])|(17[0,6,7,8])|(19[7]))\\d{8}$";
