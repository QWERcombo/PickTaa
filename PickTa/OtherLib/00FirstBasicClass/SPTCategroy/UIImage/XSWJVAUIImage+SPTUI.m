...
//            SEL swizzledSelector = NSSelectorFromString([@"SPTimage_" stringByAppendingString:NSStringFromSelector(originalSelector)]);
//            ExchangeImplementations([self class], originalSelector, swizzledSelector);
//        }
//    });
//}

//- (NSString *)SPTimage_description {
//    return [NSString stringWithFormat:@"%@, scale = %@", [self SPTimage_description], @(self.scale)];
//}

//- (UIImage *)SPTimage_resizableImageWithCapInsets:(UIEdgeInsets)capInsets resizingMode:(UIImageResizingMode)resizingMode {
...
//        SPTLogWarn(@"UIImage (SPT)", @"UIImage (SPT) resizableImageWithCapInsets 传进来的 capInsets 的水平/垂直方向的和应该小于图片本身的大小，否则会导致 render 时出现 invalid context 0x0 的错误。");
//    }
//    return [self SPTimage_resizableImageWithCapInsets:capInsets resizingMode:resizingMode];
//}

+ (UIImage *)spt_imageWithSize:(CGSize)size opaque:(BOOL)opaque scale:(CGFloat)scale actions:(void (^)(CGContextRef contextRef))actionBlock {
...
- (UIImage *)spt_grayImage {
    // CGBitmapContextCreate 是无倍数的，所以要自己换算成1倍
    CGSize size = self.spt_sizeInPixel;
    ...
    if (self.spt_opaque) {
        ...
        // 用 CGBitmapContextCreateImage 方式创建出来的图片，CGImageAlphaInfo 总是为 CGImageAlphaInfoNone，导致 spt_opaque 与原图不一致，所以这里再做多一步
        grayImage = [UIImage spt_imageWithSize:grayImage.size opaque:NO scale:grayImage.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)spt_imageWithAlpha:(CGFloat)alpha {
    return [UIImage spt_imageWithSize:self.size opaque:NO scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawInRect:CGRectMakeWithSize(self.size) blendMode:kCGBlendModeNormal alpha:alpha];
    }];
}

- (UIImage *)spt_imageWithTintColor:(UIColor *)tintColor {
    return [UIImage spt_imageWithSize:self.size opaque:self.spt_opaque scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)spt_imageWithBlendColor:(UIColor *)blendColor {
    UIImage *coloredImage = [self spt_imageWithTintColor:blendColor];
...
- (UIImage *)spt_imageWithImageAbove:(UIImage *)image atPoint:(CGPoint)point {
    return [UIImage spt_imageWithSize:self.size opaque:self.spt_opaque scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)spt_imageWithSpacingExtensionInsets:(UIEdgeInsets)extension {
    ...
    return [UIImage spt_imageWithSize:contextSize opaque:self.spt_opaque scale:self.scale actions:^(CGContextRef contextRef) {
        [self drawAtPoint:CGPointMake(extension.left, extension.top)];
    }];
}

- (UIImage *)spt_imageWithClippedRect:(CGRect)rect {
...
- (UIImage *)spt_imageWithClippedCornerRadius:(CGFloat)cornerRadius {
    return [self spt_imageWithClippedCornerRadius:cornerRadius scale:self.scale];
}

- (UIImage *)spt_imageWithClippedCornerRadius:(CGFloat)cornerRadius scale:(CGFloat)scale {
    if (cornerRadius <= 0) {
        return self;
    }
    return [UIImage spt_imageWithSize:self.size opaque:NO scale:scale actions:^(CGContextRef contextRef) {
        [[UIBezierPath bezierPathWithRoundedRect:CGRectMakeWithSize(self.size) cornerRadius:cornerRadius] addClip];
        [self drawInRect:CGRectMakeWithSize(self.size)];
    }];
}

- (UIImage *)spt_imageResizedInLimitedSize:(CGSize)size {
    return [self spt_imageResizedInLimitedSize:size resizingMode:SPTImageResizingModeScaleAspectFit];
}

- (UIImage *)spt_imageResizedInLimitedSize:(CGSize)size resizingMode:(SPTImageResizingMode)resizingMode {
    return [self spt_imageResizedInLimitedSize:size resizingMode:resizingMode scale:self.scale];
}

- (UIImage *)spt_imageResizedInLimitedSize:(CGSize)size resizingMode:(SPTImageResizingMode)resizingMode scale:(CGFloat)scale {
    ...
    if (resizingMode >= SPTImageResizingModeScaleAspectFit && resizingMode <= SPTImageResizingModeScaleAspectFillBottom) {
        CGFloat horizontalRatio = size.width / imageSize.width;
        CGFloat verticalRatio = size.height / imageSize.height;
        CGFloat ratio = 0;
        if (resizingMode >= SPTImageResizingModeScaleAspectFill && resizingMode < (SPTImageResizingModeScaleAspectFill + 10)) {
            ratio = MAX(horizontalRatio, verticalRatio);
        } else {
            // 默认按 SPTImageResizingModeScaleAspectFit
        ...
        // 默认按照 SPTImageResizingModeScaleToFill
        drawingRect = CGRectMakeWithSize(size);
        contextSize = size;
    }

    return [UIImage spt_imageWithSize:contextSize opaque:self.spt_opaque scale:scale actions:^(CGContextRef contextRef) {
        [self drawInRect:drawingRect];
    }];
}

- (UIImage *)spt_imageWithOrientation:(UIImageOrientation)orientation {
    ...
    return [UIImage spt_imageWithSize:contextSize opaque:NO scale:self.scale actions:^(CGContextRef contextRef) {
...
- (UIImage *)spt_imageWithBorderColor:(UIColor *)borderColor path:(UIBezierPath *)path {
    if (!borderColor) {
        return self;
    }
...
- (UIImage *)spt_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth cornerRadius:(CGFloat)cornerRadius {
    return [self spt_imageWithBorderColor:borderColor borderWidth:borderWidth cornerRadius:cornerRadius dashedLengths:0];
}

- (UIImage *)spt_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth cornerRadius:(CGFloat)cornerRadius dashedLengths:(const CGFloat *)dashedLengths {
    ...
    return [self spt_imageWithBorderColor:borderColor path:path];
}

- (UIImage *)spt_imageWithBorderColor:(UIColor *)borderColor borderWidth:(CGFloat)borderWidth borderPosition:(SPTImageBorderPosition)borderPosition {
    if (borderPosition == SPTImageBorderPositionAll) {
        return [self spt_imageWithBorderColor:borderColor borderWidth:borderWidth cornerRadius:0];
    } else {
        // TODO 使用bezierPathWithRoundedRect:byRoundingCorners:cornerRadii:这个系统接口
        UIBezierPath *path = [UIBezierPath bezierPath];
        if ((SPTImageBorderPositionBottom & borderPosition) == SPTImageBorderPositionBottom) {
        ...
        if ((SPTImageBorderPositionTop & borderPosition) == SPTImageBorderPositionTop) {
            [path moveToPoint:CGPointMake(0, borderWidth / 2)];
            [path addLineToPoint:CGPointMake(self.size.width, borderWidth / 2)];
        }
        if ((SPTImageBorderPositionLeft & borderPosition) == SPTImageBorderPositionLeft) {
            [path moveToPoint:CGPointMake(borderWidth / 2, 0)];
            [path addLineToPoint:CGPointMake(borderWidth / 2, self.size.height)];
        }
        if ((SPTImageBorderPositionRight & borderPosition) == SPTImageBorderPositionRight) {
...
- (UIImage *)spt_imageWithMaskImage:(UIImage *)maskImage usingMaskImageMode:(BOOL)usingMaskImageMode {
...
+ (UIImage *)spt_animatedImageWithData:(NSData *)data {
    return [self spt_animatedImageWithData:data scale:1];
}

+ (UIImage *)spt_animatedImageWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self spt_frameDurationAtIndex:i source:source];
...
+ (float)spt_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
...
+ (UIImage *)spt_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size path:(UIBezierPath *)path addClip:(BOOL)addClip {
    size = CGSizeFlatted(size);
    return [UIImage spt_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
...
+ (UIImage *)spt_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth cornerRadius:(CGFloat)cornerRadius {
    ...
    return [UIImage spt_imageWithStrokeColor:strokeColor size:size path:path addClip:NO];
}

+ (UIImage *)spt_imageWithStrokeColor:(UIColor *)strokeColor size:(CGSize)size lineWidth:(CGFloat)lineWidth borderPosition:(SPTImageBorderPosition)borderPosition {
    CGContextInspectSize(size);
    if (borderPosition == SPTImageBorderPositionAll) {
        return [UIImage spt_imageWithStrokeColor:strokeColor size:size lineWidth:lineWidth cornerRadius:0];
    } else {
        // TODO 使用bezierPathWithRoundedRect:byRoundingCorners:cornerRadii:这个系统接口
        UIBezierPath *path = [UIBezierPath bezierPath];
        if ((SPTImageBorderPositionBottom & borderPosition) == SPTImageBorderPositionBottom) {
...
+ (UIImage *)spt_imageWithColor:(UIColor *)color {
    return [UIImage spt_imageWithColor:color size:CGSizeMake(4, 4) cornerRadius:0];
}

+ (UIImage *)spt_imageWithColor:(UIColor *)color size:(CGSize)size cornerRadius:(CGFloat)cornerRadius {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);

    color = color ? color : UIColor.clearColor;
    BOOL opaque = (cornerRadius == 0.0 && [color spt_alpha] == 1.0);
    return [UIImage spt_imageWithSize:size opaque:opaque scale:0 actions:^(CGContextRef contextRef) {
...
+ (UIImage *)spt_imageWithColor:(UIColor *)color size:(CGSize)size cornerRadiusArray:(NSArray<NSNumber *> *)cornerRadius {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    ...
    return [UIImage spt_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        ...
        UIBezierPath *path = [UIBezierPath spt_bezierPathWithRoundedRect:CGRectMakeWithSize(size) cornerRadiusArray:cornerRadius lineWidth:0];
        [path addClip];
        [path fill];
    }];
}

+ (UIImage *)spt_imageWithShape:(SPTImageShape)shape size:(CGSize)size lineWidth:(CGFloat)lineWidth tintColor:(UIColor *)tintColor {
    size = CGSizeFlatted(size);
    CGContextInspectSize(size);
    ...
    return [UIImage spt_imageWithSize:size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        ...
        switch (shape) {
            ...
            break;
            case SPTImageShapeDisclosureIndicator: {
                drawByStroke = YES;
                path = [UIBezierPath bezierPath];
                path.lineWidth = lineWidth;
        ...
        if (shape == SPTImageShapeDetailButtonImage) {
...
+ (UIImage *)spt_imageWithShape:(SPTImageShape)shape size:(CGSize)size tintColor:(UIColor *)tintColor {
    CGFloat lineWidth = 0;
    switch (shape) {
        case SPTImageShapeNavBack:
            lineWidth = 2.0f;
    ...
    return [UIImage spt_imageWithShape:shape size:size lineWidth:lineWidth tintColor:tintColor];
}

+ (UIImage *)spt_imageWithAttributedString:(NSAttributedString *)attributedString {
    ...
    return [UIImage spt_imageWithSize:stringSize opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        [attributedString drawInRect:CGRectMakeWithSize(stringSize)];
    }];
}

+ (UIImage *)spt_imageWithView:(UIView *)view {
    CGContextInspectSize(view.bounds.size);
    return [UIImage spt_imageWithSize:view.bounds.size opaque:NO scale:0 actions:^(CGContextRef contextRef) {
        [view.layer renderInContext:contextRef];
    }];
}

+ (UIImage *)spt_imageWithView:(UIView *)view afterScreenUpdates:(BOOL)afterUpdates {
...
// 根据颜色创建图片
+ (UIImage *)spt_createImageWithColor:(UIColor *)color {
...
- (double)spt_getImageSize {
    int perMBBytes = 1024 * 1024;
    CGImageRef cgimage = self.CGImage;
    size_t bpp = CGImageGetBitsPerPixel(cgimage);
    size_t bpc = CGImageGetBitsPerComponent(cgimage);
    size_t bytes_per_pixel = bpp / bpc;
    double lPixelsPerMB = perMBBytes / bytes_per_pixel;
    double totalPixel = CGImageGetWidth(self.CGImage) * CGImageGetHeight(self.CGImage);
    double totalFileMB = totalPixel / lPixelsPerMB;

    return totalFileMB;
}

- (UIImage *)spt_scaledToSize:(CGSize)newSize {
