...
@implementation UIBezierPath (SPTUI)
...
+ (UIBezierPath *)spt_circleWith:(CGRect)originalFrame {
    CGRect frame = [self spt_maximumSquareFrameThatFits:originalFrame];

    UIBezierPath *path = [UIBezierPath bezierPath];
    CGFloat r = frame.size.width / 2;
    [path addArcWithCenter:CGPointMake(r, r) radius:r startAngle:0.0 endAngle:180.0 clockwise:YES];
    [path closePath];
    path.miterLimit = 4;
...
+ (UIBezierPath *)spt_chatBubble:(CGRect)originalFrame andTriHight:(CGFloat)triH andXVerExcursion:(CGFloat)xVerExcursion {
    ...
    CGFloat h = originalFrame.size.height - triH;
    UIBezierPath *bezierPath = [UIBezierPath bezierPath];
    [bezierPath moveToPoint:CGPointMake(h / 2, 0)];
    [bezierPath addCurveToPoint:CGPointMake(h / 2, h) controlPoint1:CGPointZero controlPoint2:CGPointMake(0, h)];
    [bezierPath addLineToPoint:CGPointMake((w - triH) / 2 + xVerExcursion, h)];
    [bezierPath addLineToPoint:CGPointMake(w / 2 + xVerExcursion, h + triH)];
    [bezierPath addLineToPoint:CGPointMake((w + triH) / 2 + xVerExcursion, h)];
    [bezierPath addLineToPoint:CGPointMake(w - h / 2, h)];
    [bezierPath addCurveToPoint:CGPointMake(w - h / 2, 0) controlPoint1:CGPointMake(w, h) controlPoint2:CGPointMake(w, 0)];
...
+ (UIBezierPath *)spt_couponsSepra:(CGRect)originalFrame andRadis:(CGFloat)radis {
    ...
    [bezierPath addQuadCurveToPoint:CGPointMake(radis, radis) controlPoint:CGPointMake(radis, 0)];
    [bezierPath addQuadCurveToPoint:CGPointMake(0, radis * 2) controlPoint:CGPointMake(radis, radis * 2)];
    [bezierPath addLineToPoint:CGPointMake(frame.size.width, frame.size.height)];
    [bezierPath addQuadCurveToPoint:CGPointMake(frame.size.width - radis, radis) controlPoint:CGPointMake(frame.size.width - radis, frame.size.height)];
    [bezierPath addQuadCurveToPoint:CGPointMake(frame.size.width, 0) controlPoint:CGPointMake(frame.size.width - radis, 0)];
...
+ (UIBezierPath *)spt_stars:(NSUInteger)numberOfStars shapeInFrame:(CGRect)originalFrame {
    ...
    for (int i = 0; i < numberOfStars; i++) {
        // get the star shape
        UIBezierPath *startPath = [UIBezierPath spt_starShape:babyFrame];
...
+ (CGRect)spt_maximumSquareFrameThatFits:(CGRect)frame {
