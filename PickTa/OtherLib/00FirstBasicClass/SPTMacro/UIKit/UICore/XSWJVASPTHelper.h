...
extern NSString *const _Nonnull SPT_ResourcesMainBundleName;

@interface XSWJVASPTHelper : NSObject
+ (instancetype _Nonnull)sharedInstance;
@end

@interface XSWJVASPTHelper (Bundle)
// 专属
...
@interface XSWJVASPTHelper (SystemVersion)
...
@interface XSWJVASPTHelper (DynamicType)
...
@interface XSWJVASPTHelper (Keyboard)
...
/*
 * 获取当前键盘在屏幕上的可见高度，注意外接键盘（iPad那种）时，[XSWJVASPTHelper keyboardRectWithNotification]得到的键盘rect里有一部分是超出屏幕，不可见的，如果直接拿rect的高度来计算就会与意图相悖。
 * @param notification 接收到的键盘事件的UINotification对象
 * @param view 要得到的键盘高度是相对于哪个View的键盘高度，若为nil，则等同于调用[XSWJVASPTHelper keyboardHeightWithNotification:]
...
@interface XSWJVASPTHelper (UIGraphic)
...
@interface XSWJVASPTHelper (Device)
...
@interface XSWJVASPTHelper (UIApplication)
...
@interface XSWJVASPTHelper (Animation)
...
@interface XSWJVASPTHelper (Controller)

+ (UIViewController *_Nullable)getCurrentVC;
+ (UIViewController *_Nullable)findCurrentShowingViewController;
