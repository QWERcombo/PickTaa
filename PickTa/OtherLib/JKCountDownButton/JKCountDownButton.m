    ...
    if (_timer) {
        if ([_timer respondsToSelector:@selector(isValid)])
        {
            if ([_timer isValid])
            {
                ...
                {
                    self.enabled = YES;
                    [self setTitle:kLocalizedString(@"get_ver_code", @"获取验证码") forState:UIControlStateNormal];
                    [self setTitle:kLocalizedString(@"get_ver_code", @"获取验证码") forState:UIControlStateDisabled];
