...
static CGFloat const YUFoldingMargin             = 15.0f;
static CGFloat const YUFoldingIconPadding = 10.f;
static CGFloat const YUFoldingIconWidth           = 13.5f;
...
- (void)layoutSubviews
{
    ...
    CGFloat labelWidth = self.frame.size.width - YUFoldingMargin * 2 - YUFoldingIconWidth - YUFoldingIconPadding;
    CGFloat labelHeight = self.frame.size.height;
    CGRect arrowRect = CGRectMake(YUFoldingMargin, (self.frame.size.height - 8)/2, YUFoldingIconWidth, 8);
    CGRect titleRect = CGRectMake(YUFoldingMargin + YUFoldingIconWidth + YUFoldingIconPadding, 0, labelWidth, labelHeight);
    CGRect descriptionRect = CGRectMake(YUFoldingMargin + YUFoldingIconWidth + labelWidth,  0, 0, 0);
    
    if (_arrowPosition == YUFoldingSectionHeaderArrowPositionRight) {
        arrowRect.origin.x = self.frame.size.width - YUFoldingMargin - YUFoldingIconWidth - YUFoldingIconPadding;
...
- (void)shouldExpand:(BOOL)shouldExpand {
    [UIView animateWithDuration:0.2  animations:^{
        if (shouldExpand) {
            ...
            if (self.arrowPosition == YUFoldingSectionHeaderArrowPositionRight) {
                self->_arrowImageView.transform = CGAffineTransformMakeRotation(0);
            }else{
                self.arrowImageView.transform = CGAffineTransformMakeRotation(0);
            }
        }
