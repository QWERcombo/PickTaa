...
- (void)setTagWithTagArray:(NSArray *)arr {
    if (_tagArr.count) {
        ...
        [arr enumerateObjectsUsingBlock:^(NSString *str, NSUInteger idx, BOOL *stop) {
            UIButton *tagBtn = [self viewWithTag:KBtnTag + idx];
    ...
    [arr enumerateObjectsUsingBlock:^(PTTagModel *tgeMOdel, NSUInteger idx, BOOL *stop) {
//        PTTagModel *tgeMOdel = [_tagArr objectAtIndex:idx];
        ...
        [tagBtn setTitleColor:R_G_B_16(0x999999) forState:UIControlStateNormal];
        ...
        [tagBtn setTitle:tgeMOdel.name forState:UIControlStateNormal];
        tagBtn.tag = KBtnTag + idx;
        tagBtn.layer.cornerRadius = 13;
        tagBtn.layer.borderColor = R_G_B_16(0xEEEEEE).CGColor;
        ...
        CGSize Size_str = [tgeMOdel.name sizeWithAttributes:attrs];
        ...
        tagBtn.layer.cornerRadius=tagBtn.frame.size.height/2.f;
    }];
...
- (void)tagBtnClick:(UIButton *)button {
    ...
    if (button.selected == YES) {
        button.backgroundColor = COLOR_HEX_RGB(0x5FEAC6);
